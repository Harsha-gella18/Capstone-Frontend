...
const AdminDashboard = ({ onLogout }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModal({ open: true, type: 'success', message: 'Content uploaded successfully.' });
      ...
      setModal({ open: true, type: 'error', message: err.message || 'Failed to upload content. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-gray-200">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <div className="flex items-center space-x-2">
                      <span className={`inline-flex h-2 w-2 rounded-full ${modal.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`} />
                      <p className="text-sm font-semibold text-gray-900">
                        {modal.type === 'success' ? 'Upload Successful' : 'Upload Failed'}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="px-5 pb-5">
                    <button
                      type="button"
                      onClick={() => setModal({ ...modal, open: false })}
                      className={`w-full px-4 py-2 rounded-xl font-medium text-white ${modal.type === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
                    >
                      OK
                    </button>
                  </div>
